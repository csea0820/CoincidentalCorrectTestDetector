...
public class ArffPredicateGenerator {
	...
	public ArffPredicateGenerator(String programDir, String program, int versionID) {
		this();
	...
	private TestCase parser(String gcovFile) {
		...
		int testCaseId = StringUtility
				.getDigit(fileName, fileName.length() - 3);
				...
				if (str.startsWith("branch")) {
					...
					if (values[2].equals("被执行")) {
						try {
							...
							System.err.println(program + "_v" + versionID
									+ " Test:" + testCaseId + " Line:" + str);
						}
					} else
						branchProfile.add(0);
				}
		...
		if (passed == 1 && pruneAttributes == true) {
			...
			for (Stmt s : tc.getStmts())
				if (s.isPredicate && s.getTimes() != 0)
	...
	private void generateArff(List<TestCase> tcs) {
		...
		if (pruneAttributes == false)attrCnt = getAttrSize(tcs.get(0));
		...
		for (int i = 0; i < tcs.size(); i++) {
			...
			for (int j = 0; j < tcs.get(i).getStmts().size(); j++) {
				if (tcs.get(i).getStmts().get(j).isPredicate)
					...
					if (pruneAttributes == true && af.filter(tcs.get(i).getStmts().get(j).getLineNumber()) == false)
						for (int k = 0; k < tcs.get(i).getStmts().get(j)
								.getBranchExecutionCount().size(); k++) {
						// if (j == 0 && k == 0)
						// buffer.append(tcs.get(i).getStmts().get(j).getBranchExecutionCount().get(k));
							buffer.append(","
									+ tcs.get(i).getStmts().get(j)
											.getBranchExecutionCount().get(k));
						}
		...
		writeBufferToFile(programDir + "/output_arff/v"
				+ versionID + ".arff", buffer);
	}
	...
	public void setPruneAttributes(boolean pruneAttributes) {
		this.pruneAttributes = pruneAttributes;
	}
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			System.out
					.println("Usage: ArffGenerator [programDir1 programDir2 ...]");
		} else {
			for (String s : args) {
				System.out.println("Analyzing Program " + s);
				new ArffPredicateGenerator().arffGenerator(s);
			}
		}

		// new
		// ArffGenerator().arffGenerator("/Users/csea/Documents/Experiment/Siemens/schedule2");

		// ArffGenerator ag = new
		// ArffGenerator("/Users/csea/Documents/Experiment/Siemens/print_tokens","print_tokens",
		// 1);
