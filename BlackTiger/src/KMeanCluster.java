...
public class KMeanCluster {
	...
	class Cluster {
		...
		public float calVariance() {
			...
			for (Integer integer : elements) {
				res += centroid.calVarianceOnEachTestCase(testcases
						.get(integer));
			}
		...
		public float calDistanceFromFailedTestCases() {
			...
			for (Integer integer : elements) {
				for (TestCase tc : failedTestCases) {
					...
					if (res < 0)
						System.err
								.println("long overflow in calDistanceFromFailedTestCases");
				}
			}
			return (res * 1.0f) / elements.size() / failedTestCases.size();
		}
		...
		public void updateCentroid() {
			...
			for (Integer index : elements) {
				...
				// {
				// if (st.isPredicate)
				// centroid.accumulatePredicate(st.getLineNumber(),
				// st.getTimes(),
				// st.getBranchExecutionCount());
				//
				// }
